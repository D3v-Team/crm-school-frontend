...
const ROLE_LABELS = { admin: "Administrator", teacher: "O'qituvchi", hr: "HR", cashier: "Kassir" };

export default function Create() {
    const [open, setOpen] = useState(false);
    const [showPwd, setShowPwd] = useState(false);
    const [form, setForm] = useState({ full_name: "", username: "", phone: "+998", role: "teacher", password: "" });
    const [errors, setErrors] = useState({});
    const [createUser, { isLoading }] = useCreateUserMutation();

    const handleClose = () => { setOpen(false); setForm({ full_name: "", username: "", phone: "+998", role: "teacher", password: "" }); setErrors({}); setShowPwd(false); };
    ...
    return (
        <>
            ...
            <Modal open={open} onClose={handleClose} title="Yangi xodim yaratish">
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                        ...
                        <FormField label="Rol" error={errors.role}>
                            <select className={`field-select no-icon${errors.role ? ' error' : ''}`} name="role" value={form.role} onChange={handleChange}>
                        ...
                        <FormField label="Parol" icon={Key} error={errors.password} valid={!errors.password && !!form.password}>
                            <div style={{ position: 'relative' }}>
                                ...
                                <Input icon={Key} name="password" type={showPwd ? "text" : "password"} value={form.password} onChange={handleChange}
                                    placeholder="Parol (min 6 belgi)" error={errors.password} valid={!errors.password && !!form.password} />
                                <button type="button" onClick={() => setShowPwd(p => !p)}
                                    style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', zIndex: 2 }}>
