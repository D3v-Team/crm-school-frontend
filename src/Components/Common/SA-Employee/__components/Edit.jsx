...
export default function Edit({ user }) {
    ...
    const [form, setForm] = useState({ full_name: "", username: "", phone: "+998", password: "", is_payment: false });
    ...
    const isPaymentEditable = user?.role === 'admin';
    const isSubmitting = isLoading || isUpdatingPayment;

    useEffect(() => {
        if (open && user) {
            setForm({ full_name: user.full_name || "", username: user.username || "", phone: user.phone || "+998", password: "", is_payment: Boolean(user.is_payment) });
            setErrors({});
        }
    }, [open, user]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isPaymentEditable) await updateIsPayment({ id: user.id, data: { is_payment: !!form.is_payment } }).unwrap();
    ...
    return (
        <>
            <button className="action-btn action-btn-primary" onClick={() => setOpen(true)} title="Tahrirlash"><Pencil size={14} /></button>
            <Modal open={open} onClose={() => setOpen(false)} title="Xodimni tahrirlash">
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                        ...
                        {isPaymentEditable && (
                            <label className="styled-checkbox">
                                <input type="checkbox" checked={form.is_payment} onChange={e => setForm(p => ({ ...p, is_payment: e.target.checked }))} />
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn-submit" disabled={isSubmitting}>{isSubmitting ? "Saqlanmoqda..." : "Yangilash"}</button>
                    </div>
                </form>
            </Modal>
        </>
