...
export default function Delete({ user }) {
    ...
    if (user && ["super_admin", "parent"].includes(user.role)) return null;

    const handleDelete = async () => {
        try {
            await deleteUser(user.id).unwrap();
            Alert(`${user.full_name} o'chirildi`, "success");
            setOpen(false);
        } catch (err) {
    ...
    return (
        <>
            <button className="action-btn action-btn-danger" onClick={() => setOpen(true)} title="O'chirish"><Trash size={14} /></button>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '4px 0 8px' }}>
                    ...
                    <div>
                        <p style={{ fontSize: '0.875rem', color: 'var(--text-primary)', marginBottom: 4 }}>
                            <strong style={{ color: 'var(--danger)' }}>{user.full_name}</strong> foydalanuvchini o'chirmoqchisiz.
                        </p>
