...
import { useUploadFaceMutation, useDeleteFaceMutation } from '../../../store/services/hikvision.api';
 ...
 */
export default function FaceActions({ studentId, studentName, hasFace = false }) {
    ...
    const [uploadFace, { isLoading: uploading }] = useUploadFaceMutation();
    ...
    const handleUpload = async (e) => {
        ...
        try {
            await uploadFace({ student_id: studentId, photo: file }).unwrap();
    ...
    const handleDelete = async () => {
        try {
            await deleteFace(studentId).unwrap();
    ...
    return (
        <>
            {/* Conditional button */}
            {hasFace ? (
                /* Only delete button when face exists */
                <button
                ...
                /* Only upload button when no face */
                <button
                ...
                <form onSubmit={handleUpload}>
                    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                        <p style={{ fontSize:'0.82rem', color:'var(--text-secondary)', margin:0 }}>
                            <strong style={{ color:'var(--text-primary)' }}>{studentName}</strong> uchun yuz rasmi yuklang.
                        </p>
                ...
                <div style={{ display:'flex', gap:12, alignItems:'flex-start', padding:'4px 0 8px' }}>
                    ...
                    <div>
                        <p style={{ fontSize:'0.875rem', color:'var(--text-primary)', marginBottom:4 }}>
                            <strong style={{ color:'var(--danger)' }}>{studentName}</strong> ning yuz ma'lumotini o'chirmoqchisiz.
                        </p>
