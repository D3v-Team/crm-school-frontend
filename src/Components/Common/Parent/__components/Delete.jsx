...
export default function Delete({ user }) {
    ...
    const handleDelete = async () => {
        try { await deleteUser(user.id).unwrap(); Alert(`${user.full_name} o'chirildi`,"success"); setOpen(false); }
                ...
                <div style={{ display:'flex', gap:12, alignItems:'flex-start', padding:'4px 0 8px' }}>
                    ...
                    <div>
                        <p style={{ fontSize:'0.875rem',color:'var(--text-primary)',marginBottom:4 }}>
                            <strong style={{ color:'var(--danger)' }}>{user.full_name}</strong> ota-onani o'chirmoqchisiz.
                        </p>
